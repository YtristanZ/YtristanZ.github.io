...
window.onload = function initTriangles(){
	...
	document.getElementById("update-gasket").addEventListener("click", function() {
		var inputValue = parseInt( document.getElementById("subdivision-level").value);
		var inputtheta = parseInt( document.getElementById("subdivision-theta").value);
		if(!isNaN(inputValue) && inputValue>0 && inputValue <=7 && inputtheta<=240) {
...
function initGasket(){
	...
	var vertices = [
		...
		// radius * Math.cos(30 * Math.PI / 180.0), radius * Math.sin(30 * Math.PI / 180.0),  0,       // 30度
        // radius * Math.cos(150 * Math.PI / 180.0), radius * Math.sin(150 * Math.PI / 180.0),  0,      // 150度
